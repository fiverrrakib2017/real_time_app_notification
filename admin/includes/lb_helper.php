...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = '6A7B37E9';
		$this->api_url = 'https://license.thivapro.com/';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["f8f48ead1ed0ecd"])){
				$_SESSION["f8f48ead1ed0ecd"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["f8f48ead1ed0ecd"])){
				$get_data = $this->call_api(
					'POST',
